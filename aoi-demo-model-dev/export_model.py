...
def export_caffe2_tracing(cfg, torch_model, inputs):
    ...
    if args.format == "caffe2":
        ...
        caffe2_model.save_graph(os.path.join(
            args.output, "model.svg"), inputs=inputs)
        return caffe2_model
    ...
    if isinstance(torch_model, GeneralizedRCNN):
        ...
        class ScriptableAdapter(ScriptableAdapterBase):
            def forward(self, inputs: Tuple[Dict[str, Tensor]]) -> List[Dict[str, Tensor]]:
                ...
                return [{
                    "image_size" : torch.tensor(i.image_size, dtype=torch.int64), # height, width
                    "pred_boxes": i.pred_boxes.tensor,
                    "scores": i.scores,
                    "pred_classes": i.pred_classes,
                    "pred_sub_classes": i.pred_sub_classes
                } for i in instances]
    else:
        class ScriptableAdapter(ScriptableAdapterBase):
            def forward(self, inputs: Tuple[Dict[str, Tensor]]) -> List[Dict[str, Tensor]]:
...
def export_tracing(torch_model, inputs):
    ...
    def eval_wrapper(inputs):
        ...
        instances = traceable_model.outputs_schema(
            ts_model(input["image"]))[0]["instances"]
        postprocessed = detector_postprocess(
            instances, input["height"], input["width"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample-image", default=None,
                        type=str, help="sample image for input")
    parser.add_argument("--data-path", default=None, type=str,
                        metavar="PATH", help="path to root of test data. If no image provided. The first image of this dataset will be used for tracing input")
    parser.add_argument("--run-eval", action="store_true")
    parser.add_argument(
        "--output", help="output directory for the converted model")
    parser.add_argument(
        "opts",
    ...
    if args.data_path is not None:
        ...
        meta_dict = get_meta_info(
            test_ann_path, data_config_path, sub_class_list)
        register_custom_coco_instances(
